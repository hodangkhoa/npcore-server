AddEventHandler('chatMessage', function(source, name, message)
  ...
  --[[TriggerEvent('es:addCommand', 'me', function(source, args, user)
    local name = getIdentity(source)
    table.remove(args, 2)
    TriggerClientEvent('npc-qalle-chat:me', -1, source, name.firstname, table.concat(args, " "))
end)
...
AddEventHandler("911", function(args, cid)
    ...
    exports.ghmattimysql:execute('SELECT `job`, `first_name`, `last_name`, `phone_number` FROM __characters WHERE id = ?', {cid}, function(result)
        ...
        for k,v in pairs(GetPlayers()) do
            TriggerClientEvent('npc-911:display', k, firstname, lastname, phonenumber, tostring(message), src)
        end
    end)
end)
...
AddEventHandler("311", function(args, cid)
    ...
    exports.ghmattimysql:execute('SELECT `job`, `first_name`, `last_name`, `phone_numbers` FROM __characters WHERE id = ?', {cid}, function(result)
        ...
        for k,v in pairs(GetPlayers()) do
            TriggerClientEvent('npc-311:display', k, firstname, lastname, phonenumber, tostring(message), src)
        end
    end)
end)
...
AddEventHandler("911r", function(args, cid)
    ...
    exports.ghmattimysql:execute('SELECT `job`, `first_name`, `last_name`, `phone_number` FROM __characters WHERE id = ?', {cid}, function(result)
        ...
        for k,v in pairs(GetPlayers()) do
            TriggerClientEvent('npc-911r:display', k, target, firstname, lastname, phonenumber, tostring(message))
        end
...
AddEventHandler("311r", function(args, cid)
    ...
    exports.ghmattimysql:execute('SELECT `job`, `first_name`, `last_name`, `phone_number` FROM __characters WHERE id = ?', {cid}, function(result)
        ...
        for k,v in pairs(GetPlayers()) do
            local player = exports['npc-core']:GetCurrentCharacterInfo(k)
            TriggerClientEvent('npc-311r:display', k, target, firstname, lastname, phonenumber, tostring(message))
        end
