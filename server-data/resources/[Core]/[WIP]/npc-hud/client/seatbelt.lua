  ...
  Citizen.Wait(0)
    ...
    if car ~= 0 and (wasInCar or IsCar(car)) then
      ...
      if IsControlJustReleased(0, 29) and GetLastInputMethod(0) then
        beltOn = not beltOn 
        if beltOn then 
		  TriggerEvent("pNotify:SendNotification", {text = "Seatbelt Enabled", type = "success", timeout = 1400, layout = "centerLeft"})
		  ...
		  TriggerEvent("pNotify:SendNotification", {text = "Seatbelt Disabled", type = "error", timeout = 1400, layout = "centerLeft"}) 
